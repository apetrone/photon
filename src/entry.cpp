...
#define VIEWPORT_WIDTH 320.0f
#define VIEWPORT_HEIGHT 240.0f
...
//	modelview = glm::translate( modelview, glm::vec3( 0, 0, 150 ));
	...
	glm::mat4 rotate = glm::rotate( glm::mat4(1.0), 45.0f, glm::vec3(0,1,0) );
	
	// origin in the top left
//	projection = glm::ortho( 0.0f, (float)VIEWPORT_WIDTH, (float)VIEWPORT_HEIGHT, 0.0f, -0.1f, 128.0f );
//	projection = glm::perspective( 60.0f, ((float)VIEWPORT_WIDTH/(float)VIEWPORT_HEIGHT), 0.1f, 128.0f );
...
//	for( int h = 0; h < VIEWPORT_HEIGHT; ++h )
	{
		for( int w = 0; w < VIEWPORT_WIDTH; ++w )
		{
			...
			if ( 1 ) // normalized device coordinate space
			{
